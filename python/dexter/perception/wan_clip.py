...
def flash_attention(q, k, v, num_heads, causal=False, **kwargs):
    """SDPA stand-in for upstream's flash_attn entry point.

    Upstream takes [B, L, N*C] and reshapes internally; keep that contract so
    the vendored call sites are untouched. flash-attn has no ROCm wheel for
    gfx1151, and at 257 CLIP tokens the kernel choice is immaterial.

    Extra keyword arguments (``compatibility_mode``, dropout, window sizes) are
    accepted and ignored: they select between flash-attn entry points upstream
    and mean nothing to SDPA, which already picks its own backend.
    """
    ...
    return out.transpose(1, 2).reshape(b, lq, num_heads * head_dim)
